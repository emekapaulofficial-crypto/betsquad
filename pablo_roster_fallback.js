...
(function(){
  ...
  function same(a,b){const x=norm(a),y=norm(b);return x&&y&&(x===y||x.includes(y)||y.includes(x));}
  ...
  function install(){
    ...
    window.openRoomTeam=async function(){
      ...
      try{
        ...
        const teamsQ=await window.supabase.from('fp_teams').select('id,name,logo_url');
        const teams=(teamsQ.data||[]).filter(t=>clubs.some(c=>same(t.name,c)));
        const ids=teams.map(t=>t.id).filter(Boolean);if(!ids.length)return;
        ...
        if(q.error)return;
        const seen=new Set((R.players||[]).map(p=>`${String(p.name).toLowerCase()}|${norm(p.club)}|${pos(p.position)}`));
        for(const p of (q.data||[])){
          const club=p.fp_teams?.name||'';const position=pos(p.position);const key=`${String(p.name).toLowerCase()}|${norm(club)}|${position}`;
