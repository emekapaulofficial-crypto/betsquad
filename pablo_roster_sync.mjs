...
const leagues = [
  'eng.1','esp.1','ita.1','ger.1','fra.1','ned.1','por.1','usa.1','uefa.champions','uefa.europa'
];
...
function position(a) {
  const v = String(a?.position?.abbreviation || a?.position?.name || a?.position?.displayName || '').toUpperCase();
  if (/^(GK|G|GOAL)/.test(v)) return 'GK';
  if (/^(D|DF|DEF|BACK)/.test(v)) return 'DEF';
  if (/^(M|MF|MID|WING)/.test(v)) return 'MID';
  if (/^(F|FW|ST|FORWARD|ATT)/.test(v)) return 'ST';
  return null;
}
...
async function upsertTeam(team) {
  const rows = await sb(`fp_teams?provider=eq.espn&provider_team_id=eq.${encodeURIComponent(team.id)}&select=id`, { method: 'GET' });
  ...
  if (rows?.[0]?.id) {
    await sb(`fp_teams?id=eq.${rows[0].id}`, { method:'PATCH', body:JSON.stringify(body), headers:{Prefer:'return=minimal'} });
    return rows[0].id;
  }
  const created = await sb('fp_teams', { method:'POST', body:JSON.stringify(body), headers:{Prefer:'return=representation'} });
  return created?.[0]?.id;
}

async function upsertPlayer(p, team) {
  const teamId = await upsertTeam(team);
  if (!teamId || !p?.id) return false;
  const pos = position(p);
  if (!pos) return false;
  const body = {
    provider:'espn', provider_player_id:Number(p.id), team_id:teamId,
    name:p.displayName || p.fullName || p.shortName || 'Unknown player',
    position:pos, number:p.jersey ? Number(p.jersey) : null,
    photo_url:p.headshot?.href || null, active:true, updated_at:new Date().toISOString()
  };
  ...
  if (existing?.[0]?.id) {
    await sb(`fp_players?id=eq.${existing[0].id}`, {method:'PATCH',body:JSON.stringify(body),headers:{Prefer:'return=minimal'}});
  } else {
    await sb('fp_players',{method:'POST',body:JSON.stringify(body),headers:{Prefer:'return=minimal'}});
  }

  // Keep the application's existing players table populated so the normal picker works.
    ...
    try {
      ...
      for (const event of (data.events || [])) {
        ...
        for (const comp of (event.competitions?.[0]?.competitors || [])) {
          ...
          const roster = await espn(`${league}/teams/${t.id}/roster`).catch(()=>null);
          if (!roster) continue;
          const athletes = flattenAthletes(roster);
          for (const p of athletes) if (await upsertPlayer(p,team)) players++;
          teams++;
        }
      }
    } catch (e) {
      console.warn(`Pablo could not research ${league}: ${e.message}`);
    }
  }
