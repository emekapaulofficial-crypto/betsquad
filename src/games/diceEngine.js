...
/** Four-round head-to-head Dice game. Each player rolls two dice exactly four times. Highest cumulative total wins. */
export function createDiceState(playerIds, rounds = DICE_ROUNDS) {
  ...
  return {
    playerIds: [...playerIds], rounds,
    currentRound: 1,
    currentPlayerIndex: 0,
    totals: Object.fromEntries(playerIds.map(id => [id, 0])),
    rolls: Object.fromEntries(playerIds.map(id => [id, []])),
    lastRoll: null,
    status: 'playing',
    winner: null,
    tie: false
...
export function currentDicePlayer(state) {
  return state.playerIds[state.currentPlayerIndex];
}

/** Record one player's roll for the current round. No Pig scoring or turn-loss rule applies. */
export function applyDiceRoll(state, playerId, values) {
  ...
  if (!Array.isArray(values) || values.length !== 2 || values.some(v => !Number.isInteger(v) || v < 1 || v > 6)) {
    throw new Error('Dice requires exactly two six-sided dice');
  }
  const total = values[0] + values[1];
  state.lastRoll = { playerId, round: state.currentRound, values: [...values], total };
  state.rolls[playerId].push({ round: state.currentRound, values: [...values], total });
  state.totals[playerId] += total;

  if (state.currentPlayerIndex === state.playerIds.length - 1) {
    if (state.currentRound >= state.rounds) {
      const sorted = [...state.playerIds].sort((a, b) => state.totals[b] - state.totals[a]);
      const high = state.totals[sorted[0]];
      const winners = sorted.filter(id => state.totals[id] === high);
      state.status = 'finished';
      state.winner = winners.length === 1 ? winners[0] : null;
      state.tie = winners.length > 1;
      return { event: state.tie ? 'tie' : 'winner', round: state.currentRound, total, winner: state.winner, winners, finalTotals: { ...state.totals } };
    }
...
// Compatibility helpers retained for callers that still import the old API.
...
export function holdPigTurn(state, playerId) {
  if (!state || state.status !== 'playing') throw new Error('Game is finished');
  if (currentDicePlayer(state) !== playerId) throw new Error("Not this player's turn");
  throw new Error('This Dice game has no Hold rule. Each player rolls four times.');
}
export function getPigTurn(state) { return currentDicePlayer(state); }
export function rankPigPlayers(state) {
  return [...state.playerIds].map(pid => ({ pid, score: state.totals[pid] })).sort((a, b) => b.score - a.score);
}
