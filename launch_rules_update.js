...
(function(){
  ...
  async function getFixtures(){
    // Do not require a particular status value: existing fixture imports can use
    // scheduled/upcoming/published. Date is the source of truth for this screen.
  ...
  window.showFastRoomMatches=async function(){
    ...
    main.innerHTML=`<button class="back" onclick="backToRoom()">← Back to room</button>
      <div class="section"><div><span class="badge">MATCH SELECTION</span><h2>Choose today's or tomorrow's match</h2><p class="muted">Matches appear immediately from the FootballPoints fixture database. Times are Nigeria time.</p></div></div>
    ...
    const draw=(fs)=>{
      ...
      const t=todayKey(),tm=tomorrowKey();
      const group=(k,title)=>{const rows=fs.filter(f=>dayKey(f.kickoff_at)===k);return `<section class="fp-match-day"><h3>${title}</h3>${rows.length?rows.map(f=>`<div class="fp-match-row"><div><b>${esc(f.home_team)} vs ${esc(f.away_team)}</b><div class="small">${dateLabel(f.kickoff_at)} • ${time(f.kickoff_at)} (Nigeria)</div></div><button class="primary" onclick="selectRoomFixture('${esc(f.id)}')">SELECT MATCH</button></div>`).join(''):'<div class="notice">No match stored for this day yet.</div>'}</section>`;};
    ...
    try{
      ...
      // Background refresh is optional. It must never block or erase cached matches.
  ...
  // FINAL requested selection rule: exactly 7 players.
