...
(function(){
  ...
  function init(){
    ...
    window.openMatch=async function(id){
      const f=currentFixture(id); if(!f)return alert("Match not found.");
      state.selectedFixtures=[f];state.page="matchDetail";await window.render();
    };
    ...
    window.matchDetailPage=async function(){
      ...
      const players=error?[]:(data||[]),byClub={};
      players.forEach(p=>(byClub[p.club]??=[]).push(p));
      return `<button class="back" onclick="go('matches')">← Back to upcoming matches</button>
      <div class="section"><div><span class="badge">MATCH</span><h2>${esc(f.home_team)} vs ${esc(f.away_team)}</h2><p class="muted" style="margin:0">🕒 ${esc(fixtureTime(f))}</p></div><button class="primary" onclick="buildTeamForMatch('${esc(f.id)}')">Pick players for this match →</button></div>
      <div class="notice"><b>Players for this match</b><br><span class="muted">Only active players from ${esc(f.home_team)} and ${esc(f.away_team)} are shown here. Confirmed starting-XI data can replace this pool when the football-data provider supplies the lineup.</span></div>
    ...
    window.buildTeamForMatch=async function(id){
      const f=currentFixture(id);if(!f)return alert("Match not found.");
      state.selectedFixtures=[f];await window.start("league");
    };

    const originalRender=window.render,originalGo=window.go,originalSubmitTeam=window.submitTeam;
    window.go=function(p){
      if(p==="matchDetail"){state.page="matchDetail";state.menuOpen=false;return window.render();}
      return originalGo(p);
    };
      ...
      if(state.page!=="matchDetail"){
        ...
        if(state.page==="builder"&&state.selectedFixtures?.length){
          ...
          if(main&&!main.querySelector(".selected-match-time")){
            const box=document.createElement("div");box.className="notice selected-match-time";box.style.marginBottom="16px";
            box.innerHTML=`<b>Selected match time:</b> ${state.selectedFixtures.map(f=>`${esc(f.home_team)} vs ${esc(f.away_team)} — ${esc(fixtureTime(f))}`).join("<br>")}`;main.prepend(box);
          }
        }
        return;
      }
      const page=await window.matchDetailPage();
      const old=state.page;state.page="home";await originalRender();state.page=old;
      const main=document.querySelector("main.wrap");if(main)main.innerHTML=page;
    };
