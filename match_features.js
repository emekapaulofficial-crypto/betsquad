...
(function(){
  ...
  function init(){
    ...
    async function rosterForClubs(clubs,f){
      let players=[];
      const first=await supabase.from("players").select("id,name,club,position,photo_url,team_provider_id").eq("active",true).in("club",clubs).order("club").order("name");
      if(!first.error)players=first.data||[];
      if(players.length<teamSize()){
        try{
          const league=leagueCodeFromFixture(f);
          const teams0=await supabase.from("fp_teams").select("id,name,provider_team_id,logo_url");
          const allTeams=teams0.data||[];
          const matched=allTeams.filter(t=>clubs.some(c=>sameClub(t.name,c)));
          const map={};for(const t of matched){for(const c of clubs)if(sameClub(t.name,c))map[String(c).toLowerCase()]=t.provider_team_id;}
      ...
      const teams=await supabase.from("fp_teams").select("id,name,provider_team_id,logo_url");
      const matchedTeams=(teams.data||[]).filter(t=>clubs.some(c=>sameClub(t.name,c)));
      const teamIds=matchedTeams.map(t=>t.id).filter(Boolean);
      const teamNameById=new Map(matchedTeams.map(t=>[String(t.id),t.name]));
      if(teamIds.length){
        const second=await supabase.from("fp_players").select("id,name,position,photo_url,team_id,number").eq("active",true).in("team_id",teamIds).order("name");
        if(!second.error){
          const found=new Set(players.map(p=>`${String(p.name).toLowerCase()}|${String(p.club).toLowerCase()}`));
          for(const p of (second.data||[])){
            const club=teamNameById.get(String(p.team_id))||"";
        ...
        }else console.warn("fp_players read failed:",second.error.message);
      }
      if(players.length<teamSize()){
        const providerIds=matchedTeams.map(t=>t.provider_team_id).filter(Boolean);
        if(providerIds.length){
          const third=await supabase.from("players").select("id,name,club,position,photo_url,team_provider_id").eq("active",true).in("team_provider_id",providerIds).order("club").order("name");
          if(!third.error){const found=new Set(players.map(p=>String(p.id)));for(const p of(third.data||[]))if(!found.has(String(p.id))){players.push(p);found.add(String(p.id));}}
        }
      }
      return players.filter(p=>clubs.some(c=>sameClub(p.club,c)));
    }
    window.matchDetailPage=async function(){
      ...
      const clubHtml=clubs.map(club=>{const rows=players.filter(p=>sameClub(p.club,club));return `<div class="panel"><h3>${esc(club)}</h3>${rows.map(p=>{const picked=isSelected(p.id),full=!picked&&positionComplete(p.position);return `<button type="button" class="row" style="width:100%;text-align:left;cursor:${full?"not-allowed":"pointer"};opacity:${full&&!picked?".55":"1"};background:${picked?"rgba(80,214,143,.12)":"transparent"};border:1px solid ${picked?"rgba(80,214,143,.55)":"rgba(120,150,190,.25)"};margin-bottom:8px" onclick="toggleMatchPlayer('${esc(p.id)}')" ${full&&!picked?"disabled":""}>${p.photo_url?`<img src="${esc(p.photo_url)}" alt="" style="width:36px;height:36px;border-radius:50%;object-fit:cover;margin-right:10px">`:`<span style="display:inline-flex;width:36px;height:36px;border-radius:50%;align-items:center;justify-content:center;background:#16304b;margin-right:10px">⚽</span>`}<div style="flex:1"><b>${esc(p.name)}</b><div class="small">${esc(p.position||"Player")}${p.number?` · #${esc(p.number)}`:""}</div></div><span class="badge">${picked?"✓ PICKED":full?"FULL":"PICK"}</span></button>`;}).join('')||`<p class="muted">Pablo is still syncing the verified roster for this team.</p>`}</div>`;}).join("");
      return `<button class="back" onclick="go('matches')">← Back to upcoming matches</button><div class="section"><div><span class="badge">MATCH</span><h2>${esc(f.home_team)} vs ${esc(f.away_team)}</h2><p class="muted" style="margin:0">🕒 ${esc(fixtureTime(f))}</p></div><button class="primary" onclick="submitMatchTeam()" ${complete?"":"disabled"} style="opacity:${complete?"1":".6"}">${complete?`Save ${teamSize()} players for this match →`:`Pick ${teamSize()-selected.length} more player(s)`}</button></div>${selectedHtml}<div class="notice"><b>Players for this match</b><br><span class="muted">Pablo's verified roster is checked automatically. Only players belonging to these two teams are shown.</span></div><div class="two" style="margin-top:16px">${clubHtml}</div>`;
    };
