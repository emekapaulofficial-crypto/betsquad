...
(function(){
  ...
  function init(){
    ...
    async function rosterForClubs(clubs,f){
      ...
      const teams=await supabase.from("fp_teams").select("id,name,logo_url").in("name",clubs);const teamIds=(teams.data||[]).map(t=>t.id).filter(Boolean);if(!teamIds.length)return players;
    ...
    window.matchDetailPage=async function(){
      ...
      const clubs=[f.home_team,f.away_team].filter(Boolean);const players=await rosterForClubs(clubs,f);state.matchPlayers=players;const byClub={};players.forEach(p=>{const key=p.club||"Unknown team";(byClub[key]??=[]).push(p);});const n=need(),selected=state.selected||[];const selectedByPos=Object.fromEntries(Object.keys(n).map(k=>[k,selected.filter(p=>p.position===k).length]));const complete=Object.keys(n).every(k=>selectedByPos[k]===n[k]);
