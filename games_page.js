/* Games hub: wires the "Games" nav page to the existing rule engines in
   src/games/*.js, which previously existed but were never loaded anywhere.
   This file owns everything for state.page === 'game_room' by intercepting
   window.render (same pattern room_match_final_fix.js uses for its own
   pages) — it does not touch window.signIn/signUp/signOut. */
...
const BOT_WAIT_MS = 15000; // how long to wait for humans before offering a bot seat
...
window.playGame=async function(type){
  const s=window.state; if(!s.user)return window.go?window.go('auth'):(s.page='auth',window.render());
  const rules=RULES[type]; if(!rules)return;
  const sb=window.supabase;
  try{
    let room=null;
    const {data:openRooms}=await sb.from('game_rooms').select('id,capacity,status').eq('game_type',type).eq('status','waiting').order('created_at',{ascending:true}).limit(5);
    for(const r of (openRooms||[])){
      const {count}=await sb.from('game_room_players').select('id',{count:'exact',head:true}).eq('room_id',r.id);
      if((count||0)<r.capacity){room=r;break;}
    }
    if(!room){
      const {data:created,error:ce}=await sb.from('game_rooms').insert({game_type:type,stake:rules.entryFee,capacity:rules.capacity,creator_id:s.user.id}).select().single();
      if(ce){alert('Could not create a room: '+ce.message);return;}
      room=created;
    }
    const {data:roomCheck}=await sb.from('game_rooms').select('status').eq('id',room.id).single();
    ...
    const {error:je}=await sb.from('game_room_players').insert({room_id:room.id,user_id:s.user.id,display_name:displayName});
    if(je){alert('Could not join room: '+je.message);return;}
    ...
    s.gameType=type; s.gameRoomId=room.id; s.gameWaitStarted=Date.now(); s.gameRoomCache=null; s.gameSettlement=null; s.page='game_room';
    window.render();
    startGamePolling();
  }catch(e){alert('Something went wrong starting the game: '+(e?.message||e));}
};
...
window.leaveGameRoom=async function(){
  ...
  stopGamePolling();
  s.gameRoomId=null; s.gameRoomCache=null; s.gameSettlement=null; s.page='games';
  window.render();
};
...
async function tick(){
  ...
  if(!room){ stopGamePolling(); return; }
...
function readyCheck(gameType,players,waitStartedAt){
  ...
  // whot / dice — capacity 4
  if(total>=4)return{ready:true,needBot:false};
  if(!canStartRoom(humans))return{ready:false};
  if(!waited)return{ready:false};
  return{ready:true,needBot:shouldFillWithHouseAi(humans,total>=4)};
}

/* ---------- Game initialisation (run once by the room leader) ---------- */
async function addBot(room){
  const {data:bot}=await window.supabase.from('game_room_players').insert({room_id:room.id,is_bot:true,display_name:BOT_RULES.name}).select().single();
  return bot;
}
...
async function initWhot(room,players,needBot){
  ...
  await sb.from('game_rooms').update({status:'active',state:newState,started_at:new Date().toISOString()}).eq('id',room.id).eq('status','waiting');
}
...
async function initDice(room,players,needBot){
  ...
  const pids=allPlayers.map(p=>p.user_id||p.id);
  const rolls={};
  if(bot) rolls[bot.id]=rollDice(2,6); // bot rolls immediately so humans aren't left waiting on it
...
window.playWhotCard=async function(idx){
  ...
  if(card && card.value===20){
    declaredSuit=prompt('Whot! Declare a suit: '+SUITS.join(', '));
    if(!SUITS.includes(declaredSuit))return alert('Invalid suit.');
  }
...
window.drawWhotCard=async function(){
  ...
  state.hands[pid]=[...(state.hands[pid]||[]),card];
  state.turnPlayerId=nextPlayer(state.turnOrder,pid);
  await sb.from('game_rooms').update({state}).eq('id',room.id);
  renderGameRoom();
  maybeBotMove(room.id);
...
async function maybeFinishDice(roomId){
  ...
  const {winners}=resolveHighestTotal(results);
  const winnerPids=winners.map(w=>w.pid);
  await sb.from('game_rooms').update({state:{...state,status:'finished'}}).eq('id',roomId);
  const humanWinners=winnerPids.filter(pid=>!isBotPid(pid)).slice(0,2);
  await settleRoom(roomId,humanWinners);
}
...
window.snookerPot=async function(colour){
  ...
  state.currentPlayerId=state.playerIds.find(id=>id!==pid);
  await sb.from('game_rooms').update({state}).eq('id',room.id);
  renderGameRoom();
  maybeBotMove(room.id);
};
...
function maybeBotMove(roomId){
  ...
  setTimeout(async()=>{
    const {data:room}=await sb.from('game_rooms').select('*').eq('id',roomId).single();
    if(!room||room.status!=='active')return;
    const botPlayer=(window.state.gameRoomPlayers||[]).find(p=>p.is_bot);
    if(!botPlayer)return;
    const botPid=botPlayer.id, st=room.state;
    if(room.game_type==='whot'){
      if(st.turnPlayerId!==botPid)return;
      const hand=st.hands[botPid]||[];
      const i=hand.findIndex(c=>canPlay(c,st.topCard,st.declaredSuit));
      if(i===-1){
        if(st.drawPile.length){ const card=st.drawPile.shift(); st.hands[botPid]=[...hand,card]; }
        st.turnPlayerId=nextPlayer(st.turnOrder,botPid);
        await sb.from('game_rooms').update({state:st}).eq('id',roomId);
        renderGameRoom();
        return;
      }
      const card=hand[i]; const newHand=hand.filter((_,idx)=>idx!==i);
      st.hands[botPid]=newHand; st.topCard=card;
      st.declaredSuit=card.value===20?SUITS[secureInt(0,SUITS.length-1)]:null;
      if(newHand.length===0){
        await sb.from('game_rooms').update({state:{...st,status:'finished'}}).eq('id',roomId);
    ...
    if(room.game_type==='snooker'){
      ...
      const colours=Object.keys(BALL_VALUES);
      const colour=colours[secureInt(0,colours.length-1)];
      try{ scorePot(st,botPid,colour); }catch(e){}
      st.currentPlayerId=st.playerIds.find(id=>id!==botPid);
      await sb.from('game_rooms').update({state:st}).eq('id',roomId);
      renderGameRoom();
    }
  },1200);
...
function renderActiveGame(room,players){
  ...
  if(room.game_type==='whot'){
    const hand=state.hands?.[pid]||[];
    const myTurn=state.turnPlayerId===pid;
  ...
  if(room.game_type==='dice'){
    const mine=state.rolls?.[pid];
    const waiting=(state.order||[]).filter(p=>!state.rolls[p]).length;
  ...
  if(room.game_type==='snooker'){
    const myTurn=state.currentPlayerId===pid;
    const scoreLine=Object.entries(state.scores||{}).map(([k,v])=>`${esc(findPlayerName(players,k))}: ${v}`).join(' • ');
