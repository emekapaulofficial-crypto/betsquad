/* Personal Wallet dashboard: private balance/history and automatic withdrawal-status refresh. */
(function(){
  'use strict';
  let wrapped=false;
  ...
  function dashboardHtml(){
    return '<section id="fpPersonalDashboard" class="panel" style="display:block!important;visibility:visible!important;opacity:1!important;position:relative;z-index:30;margin:16px auto;padding:18px;border:2px solid rgba(82,224,145,.45);background:#0b1d31;scroll-margin-top:90px">'+
      ...
      '<div id="fpWithdrawalBox" class="notice" style="margin-top:12px"><h3 style="margin-top:0">Request Withdrawal</h3><p class="small muted">Enter the amount you want to withdraw and the bank/account where you want it sent.</p><input id="fpWithdrawAmount" type="number" min="1" step="0.01" placeholder="Amount"><input id="fpWithdrawDestination" placeholder="Bank + account number" style="margin-top:8px"><button id="fpWithdrawButton" class="primary" style="width:100%;margin-top:8px">REQUEST WITHDRAWAL</button></div>'+
    '</section>';
  }
  ...
  async function loadDashboard(){
    ...
    if(!box||!s?.user||!sb||s.page!=='wallet')return;
    const uid=s.user.id;
    try{
      ...
      const pending=withdrawals.find(x=>String(x.status).toLowerCase()==='pending');
      const approved=withdrawals.find(x=>String(x.status).toLowerCase()==='approved');
      box.querySelector('#fpDashBody').innerHTML=
        ...
        (pending?'<div class="notice" style="margin-top:12px"><b>WITHDRAWAL PENDING</b><br>'+fmt(pending.amount)+' — waiting for Admin approval.</div>':'')+
      ...
      const form=box.querySelector('#fpWithdrawalBox');
      if(pending){
        form.innerHTML='<h3 style="margin-top:0">Withdrawal request submitted</h3><p class="small muted">Your '+fmt(pending.amount)+' withdrawal is pending Admin approval. You cannot submit another withdrawal until this one is resolved.</p>';
      } else {
        if(!form.querySelector('#fpWithdrawButton')){
          form.innerHTML='<h3 style="margin-top:0">Request Withdrawal</h3><p class="small muted">Enter the amount you want to withdraw and the bank/account where you want it sent.</p><input id="fpWithdrawAmount" type="number" min="1" step="0.01" placeholder="Amount"><input id="fpWithdrawDestination" placeholder="Bank + account number" style="margin-top:8px"><button id="fpWithdrawButton" class="primary" style="width:100%;margin-top:8px">REQUEST WITHDRAWAL</button>';
          bindWithdraw(box);
        }
      }
      if(approved && !pending && form && form.dataset.approvalShown!=='1'){
        form.dataset.approvalShown='1';
        form.innerHTML='<h3 style="margin-top:0">Withdrawal completed ✓</h3><p class="small muted">Your approved withdrawal has been removed from the active request area. The completed transaction remains permanently in your Withdrawal history.</p>';
      }
    }catch(e){console.error('FootballPoints dashboard refresh',e);}
  }

  function bindWithdraw(box){
    const btn=box.querySelector('#fpWithdrawButton');
    if(!btn)return;
    btn.onclick=async()=>{
      ...
      btn.disabled=true;
      const r=await window.supabase.rpc('create_withdrawal_request',{p_amount:amount,p_destination:destination});
  ...
  async function mount(){
    ...
    const host=document.querySelector('main.wrap');if(!host)return;
    const uid=s.user.id;
    let box=document.getElementById('fpPersonalDashboard');
    if(!box||box.parentElement!==host||box.dataset.uid!==uid){
      if(box)box.remove();
      host.insertAdjacentHTML('afterbegin',dashboardHtml());
      box=document.getElementById('fpPersonalDashboard');
      box.dataset.uid=uid;
      box.querySelector('#fpDashCopyAccount').onclick=async()=>{try{await navigator.clipboard.writeText('9152926691');box.querySelector('#fpDashCopyAccount').textContent='COPIED ✓';}catch(e){alert('Account number: 9152926691')}};
      bindWithdraw(box);
    }
    await loadDashboard();
    if(refreshTimer)clearInterval(refreshTimer);
    refreshTimer=setInterval(()=>{if(window.state?.page==='wallet')loadDashboard();},3000);
  }

  function install(){
    if(wrapped||typeof window.render!=='function')return false;
    const original=window.render;
    window.render=async function(){const result=await original.apply(this,arguments);if(window.state?.page==='wallet')setTimeout(mount,0);return result;};
    wrapped=true;mount();return true;
  }
